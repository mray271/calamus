...
def test_tooltip_handler_with_mock_gvariant():
    ...
    try:
        ...
        print(f"[TEST] GVariant methods: {[m for m in dir(gvar) if not m.startswith('_')]}")
    ...
    try:
        ...
        print(f"[TEST] UserMessage methods: {[m for m in dir(msg) if not m.startswith('_') and 'get' in m.lower()]}")
...
def test_signal_signature_detection():
    ...
    try:
        ...
        signal_info = GObject.signal_lookup("script-message-received::tooltip", 
